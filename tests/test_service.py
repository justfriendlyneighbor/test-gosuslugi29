...
async def get_service_pages(session,categories):
    ...
    for category, services in categories.items():
        thisservicepages=[]
        n = 100
        urls = [[{key: ( value if key != "url" else { k: v if k != "query" else [{"key": "id", "value": service}] for k, v in value.items()}) for key, value in Service.PageUrl.items()},False] for service in services if service!='name']
        while not all([url[1] for url in urls]):
            for i in range(0, len(urls), n):
                with allure.step(f"Асинхронно сделать {n} запросов с {i} по {i+n} к страницам категории {category}"):
        ...
        for page in pages:
            with check:
                with allure.step(f'Проверить Запрос к странице {buildurl(**page["url"])}'):
                    assert page['status']==ok, f'Запрос к поиску по странице категории {buildurl(**page["url"])} вернул код отличный от {ok}, а именно {page["status"]}' 
...
def get_serviceitargetids(pages):
    ...
    for category,servicepages in pages.items():
        ...
        for servicepage in servicepages:
            with allure.step(f"Выделить подуслуги на странице услуги {buildurl(**servicepage['url'])}"):
                ...
                for section in sections:
                    ...
                    sectarg.update({sectionnames[0]:{servicetarget.attrs["data-targetid"]: {"name":' '.join([namespan.text for namespan in servicetarget.select('span[class~="service-procedure-target-link"]')])}} for servicetarget in section.select(Service.Element)})
        ...
        for serviceid,service in serviceids.items():
            for _,target in service.items():
                for targetid,targetname in target.items():
                    with check:
                        with allure.step(f"Проверить подуслугу {targetid} ({targetname['name']})"):
                            assert attribute.match(targetid), f'Подуслуга {targetid} ({targetname["name"]}) не соответствует стандартному представлению'
