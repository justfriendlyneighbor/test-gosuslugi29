...
async def get_target_authorization_pages(session):
    with aiohttp.MultipartWriter("form-data") as mp:
        ...
        with allure.step(f"Асинхронно сделать запрос к странице авторизации{buildurl(**UrlPart)}"):
...
async def get_target_pages(session,categories):
    ...
    for category, services in categories.items():
                    ...
                    if servicevariant!='name':
                        for servicetarget in servicetargets:
                            [(config["url"]["query"].clear(),
                                config["url"]["query"].extend([{"key": "id","value": servicetarget},{"key": "serviceId","value": servicename,},]),)
        ...
        for i in range(0, len(urls), n):
            with allure.step(f"Асинхронно сделать {n} запросов с {i} по {i+n} к страницам услуг"):
                tasks = [
...
def get_targetdetails(pages,categories):
    for page in pages:
        with allure.step(f"Выделить параметры на странице услуги {buildurl(**page['url'])}"):
                ...
                for servicevariant,targets in servicevariants.items():
                    if servicevariant!='name':
                        for target,details in targets.items():
                            with allure_subtests.test(subtest_name=f"Проверка подуслуги {target} ({details['name']}) услуги {serviceid} ({servicevariants['name']}) {'успешна' if details['assert']==True else f'неуспешна, не соответствуют пункты: {*[detailname for detailname,detailcontent in details.items() if detailcontent==[]],}'}"):
                                assert details['assert'], f'На странице {target} ({details["name"]}) не соответствуют пункты: {*[detailname for detailname,detailcontent in details.items() if detailcontent==[]],}'
    with allure.step(f"Прикрепить файл результатов всех проверок"):
        with open("results/data.json", "w") as f:
            allure.attach(json.dumps(request.config.categories).encode(),name='All details collection',attachment_type=allure.attachment_type.JSON)
